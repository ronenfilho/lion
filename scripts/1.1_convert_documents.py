...
Usa os extractors do pipeline (HTMLExtractor, PDFExtractor, PandocExtractor) 
que herdam de BaseExtractor, chamando extract() + save_to_markdown() para 
persistir os resultados em: data/processed/markdown/legislation/*.md

Fontes suportadas:
    .html  → HTMLExtractor  (Padrão Planalto e Padrão RFB/Normas.RFB)
    .pdf   → PDFExtractor   (via PyMuPDF)
    .doc   → PandocExtractor (via Pandoc - requer pandoc instalado)
    .docx  → PandocExtractor (via Pandoc - requer pandoc instalado)
...
Uso:
    ...
    # Converter documentos Word (.doc, .docx)
    python scripts/1.1_convert_documents.py --ext doc docx

    # Converter tudo (html + pdf + doc + docx)
...
from src.ingestion.extractors.pandoc_extractor import PandocExtractor
...
def convert_file(
    ...
    try:
        # PandocExtractor tem interface diferente: salva internamente
        if ext in {".doc", ".docx"}:
            extractor = extractor_cls(base_dir=BASE_DIR, markdown_dir=output_dir)
            result = extractor.extract(str(source), source_type="legislation")
        ...
        # HTMLExtractor e PDFExtractor usam o contrato BaseExtractor
...
Exemplos:
  python scripts/1.1_convert_documents.py --ext html
  python scripts/1.1_convert_documents.py --ext pdf
  python scripts/1.1_convert_documents.py --ext html pdf doc docx
  python scripts/1.1_convert_documents.py --file data/raw/legislation/L15270.html
  python scripts/1.1_convert_documents.py --force
        """,
    )
    ...
    parser.add_argument(
        ...
        metavar="EXT",
        default=None,
        help="Extensões a processar: html, pdf, doc, docx (padrão: todas suportadas)",
    )
    parser.add_argument(
        "--file",
        type=Path,
